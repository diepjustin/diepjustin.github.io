...
| [`ne-betting`](https://github.com/diepjustin/ne-betting) | Kalshi and Polymarket markets on Nebraska football | split out 10 Sep 2026; `uv`-managed, own `pyproject.toml`; daily collector workflow (now in that repo, cold-started since the Actions cache didn't carry over); deploys from its own `main` (branch-deploy Pages, `.nojekyll`) |
